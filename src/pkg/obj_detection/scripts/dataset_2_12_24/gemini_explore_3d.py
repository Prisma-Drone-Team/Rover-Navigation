...
class CameraCommandNode(Node):
    ...
    def command_callback(self, msg):
        if msg.data == 1 and self.image is not None:
            pose = self.get_pose()
            if pose:
                saveFlag2 = self.save_pc(self.pc)
                saveFlag = self.save_image(self.image)
                if saveFlag==True and saveFlag2==True:
    ...
    def save_pc(self, pc_msg):
        # TODO: Transform point cloud from camera to base_link before saving it
        points = []
        ...
        filename = f"/home/user/ros2_ws/src/obj_detection/scripts/dataset/pc_{self.image_counter:04d}.pcd"
        success = o3d.io.write_point_cloud(filename, pointcloud)
        if success:
            return True
        else:
            return False

...
class AgentNode(Node):
    ...
    def __init__(self, use_learned_policy=False):
        ...
        self.CLASS_TO_IDX = {"sports ball": 6, "bottle": 7}
        ...
        print(os.getcwd())
        self.agent.load_state_dict(torch.load("/home/user/ros2_ws/src/pkg/obj_agent/obj_agent/agent.cleanrl_model", map_location=self.device), strict=False)
        self.agent.eval()
        ...
        self.map_sub = self.create_subscription(OccupancyGrid,'map',self.map_callback,1,callback_group=callback_group)
    ...
    def agent_call(self):
        ...
        if self.map and self.is_navigating is False:
            ...
            if self.obj:
                ...
                for i, obj in enumerate(self.obj):
                    ...
                    if distances.size==0 or not np.any(distances<3.0): #Avoiding multiple detections of the same object type in a distance of 3 pixels
                        ...
                        print(f"Updating object {min_dist_index}")
                        self.objects[min_dist_index]["pos2D"] = (positions_px[i][1],positions_px[i][0])
            ...
            robot_px = [round(-origin[1]/self.custom_resolution + self.robot_pose[1]/self.custom_resolution), round((resized_map.shape[1] + origin[0]/self.custom_resolution)-self.robot_pose[0]/self.custom_resolution)] 
            
            #print(robot_px)
            flipped_map = np.flip(resized_map, 1)
            ...
            for obj in self.objects:
                if obj["pos2D"][1] - view_y >= 0 and obj["pos2D"][0] - view_x >= 0 and obj["pos2D"][1] - view_y < self.view_size and obj["pos2D"][0] - view_x < self.view_size:
            ...
            if self.use_learned_policy:
                if command==2:
                    ...
                    if self.robot_pose[2]==0:
                        self.robot_pose[1] = self.robot_pose[1] + 1
                    elif self.robot_pose[2]==90:
                        self.robot_pose[0] = self.robot_pose[0] + 1
                    elif self.robot_pose[2]==180:
                        self.robot_pose[1] = self.robot_pose[1] - 1
                    else: # -90 degrees
                        self.robot_pose[0] = self.robot_pose[0] - 1
                elif command==1:
                ...
                goal_msg.pose.header.frame_id = 'rover/map'
...
def main(args=None):
    ...
    agent_node = AgentNode(use_learned_policy=False)
